...
class GYBanner: UIView {
    ...
    init(frame: CGRect,imageHandle:@escaping (_ index:Int) -> ()) {
        ...
        //标题
        _titleLbl = UILabel(frame: CGRect(x: 0, y: _sv.frame.maxY - 30, width: _sv.bounds.size.width, height: 30))
        _titleLbl.font = UIFont.systemFont(ofSize: 15)
        _titleLbl.textColor = UIColor.white
        
        _titleLbl.backgroundColor = UIColor.black.withAlphaComponent(0.5)
 ...
 */
    ...
    func reloadGYBanner(_ models:Array<GYBannerModel>){
        if models.count == 0 {
            _centerView.image = UIImage(named: "1.jpg")
            return;
        }
